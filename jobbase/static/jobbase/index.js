const DOMAIN = 'https://jobbase.herokuapp.com/'
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (this.location.href === DOMAIN) {
        allPosts('allposts')
    }
    if (this.location.href === `${DOMAIN}mypage`) {
        myJobListings()
    }
    ...
    if (this.location.href === `${DOMAIN}messageview`) {
        fetch('checklogin')
        .then(response => response.json())
        .then(info => {
    ...
    if (this.location.href === `${DOMAIN}login`) {
    ...
    if (this.location.href === `${DOMAIN}register`) {
            ...
            .then(jobPosts => {
                ...
                for (let i = 0; i < jobPosts.length; i++) {
                    ...
                    document.querySelector(`#btn-${jobPosts[i].id}`).addEventListener('click', () => {
                        if (!jobPosts[i].active) {
                            this.location.href = `${DOMAIN}create-checkout-session/${jobPosts[i].id}`
                        } else {
                ...
                document.querySelector('#main1').innerHTML = `
                    ...
                    <form class="mt-4" id="postJobForm">
                        <div class="accordion" id="accordionPanelsStayOpenExample">
                            ...
                            <a href="${DOMAIN}mypage" class="btn btn-outline-secondary btn ms-2 mt-5">
                    ...
                    document.querySelector('#postJobForm').addEventListener('submit', (event) => {
                        ...
                        if (document.querySelector('#annualSalary') === null && document.querySelector('#hourlySalary') === null) {
                            ...
                            .then(id => {
                                ...
                                this.location.href = `${DOMAIN}create-checkout-session/${id}`
                            })
                        }
                    })
                }
...
function viewApplicants(id) {
    ...
    .then(applications => {
        ...
        backBtn.innerHTML = `
        <a href="${DOMAIN}mypage" id="backBtn" class="btn btn-outline-secondary btn">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-arrow-bar-left" viewBox="0 0 16 16">
                <path fill-rule="evenodd" d="M12.5 15a.5.5 0 0 1-.5-.5v-13a.5.5 0 0 1 1 0v13a.5.5 0 0 1-.5.5ZM10 8a.5.5 0 0 1-.5.5H3.707l2.147 2.146a.5.5 0 0 1-.708.708l-3-3a.5.5 0 0 1 0-.708l3-3a.5.5 0 1 1 .708.708L3.707 7.5H9.5a.5.5 0 0 1 .5.5Z"/>
            </svg>
        </a>
